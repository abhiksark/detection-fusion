...
from detection_fusion import Detection, FormatRegistry
...
def create_sample_data():
    ...
    for img_idx, image_name in enumerate(image_names):
        ...
        for model_name in ["yolov8n", "yolov8s", "yolov8m"]:
            ...
            for base_det in base_detections:
                ...
                if np.random.random() > 0.1:  # 90% detection rate
                    ...
                    # Clamp values to valid ranges before creating Detection (frozen model)
                    x = float(np.clip(base_det["x"] + np.random.normal(0, noise_scale), 0, 1))
                    y = float(np.clip(base_det["y"] + np.random.normal(0, noise_scale), 0, 1))
                    w = float(
                        np.clip(base_det["w"] + np.random.normal(0, noise_scale * 0.5), 0.01, 1)
                    )
                    h = float(
                        np.clip(base_det["h"] + np.random.normal(0, noise_scale * 0.5), 0.01, 1)
                    )
                    conf = float(np.clip(base_det["confidence"] + np.random.normal(0, 0.05), 0, 1))
            ...
            if np.random.random() > 0.7:
                fp_detection = Detection(
                    ...
                    w=float(np.random.uniform(0.05, 0.2)),
                    h=float(np.random.uniform(0.05, 0.2)),
            ...
            yolo_writer = FormatRegistry.get_writer("yolo")
            yolo_writer.write_file(model_detections, model_file)
        ...
        for base_det in base_detections:
            ...
            if np.random.random() > 0.05:
                gt_detection = Detection(
                    ...
                    w=base_det["w"],
                    h=base_det["h"],
                    confidence=1.0,  # GT doesn't have confidence scores
        ...
        if img_idx == 0:
            ...
            yolo_writer.write_file(gt_detections, gt_file)
        else:
            # Append to existing file (batch GT format)
            with open(gt_file, "a") as f:
                for det in gt_detections:
                    f.write(
                        f"{det.class_id} {det.x:.6f} {det.y:.6f} {det.w:.6f} {det.h:.6f} {det.confidence:.6f}\n"
                    )
...
def run_basic_rectification():
    ...
    rectifier.create_rectified_dataset(
        output_dir=output_dir, include_most_correct=5, include_most_incorrect=5
    )
...
def run_mode_comparison():
    ...
    conservative_rectifier = GTRectifier(
        labels_dir=labels_dir, gt_dir=gt_dir, images_dir=images_dir, mode="minimize_error"
    )
    ...
    aggressive_rectifier = GTRectifier(
        labels_dir=labels_dir, gt_dir=gt_dir, images_dir=images_dir, mode="maximize_error"
    )
    ...
    for error_type in all_error_types:
        ...
        print(
            f"     {error_type}: {conservative_count} (conservative) vs {aggressive_count} (aggressive)"
        )
    ...
    if conservative_scores and aggressive_scores:
        ...
        if different_scores:
            ...
            for img_name, cons_score, agg_score in different_scores[:5]:
                print(
                    f"     {img_name}: {cons_score:.3f} (conservative) vs {agg_score:.3f} (aggressive)"
                )
...
def analyze_rectification_results(output_dir):
    ...
    for subdir in ["most_correct", "most_incorrect"]:
        ...
        if subdir_path.exists():
            ...
            print(
                f"   {subdir}/: {image_count} images, {label_count} labels, {analysis_count} analyses"
            )
