...
def plot_variance_analysis(analyzer, top_n: int = 15, save_path: Optional[str] = None):
    ...
    ax.bar(x - width/2, high_variance['mean'], width, label='Mean Count',
           yerr=high_variance['std'], capsize=5)
...
def generate_all_plots(analyzer, output_dir: str = "plots", top_n: int = 20):
    ...
    # Generate plots
    plot_class_distribution(analyzer, top_n, 
                          save_path=str(output_path / "class_distribution.png"))
    
    plot_confidence_distribution(analyzer,
                               save_path=str(output_path / "confidence_distribution.png"))
    
    plot_model_comparison_heatmap(analyzer,
                                save_path=str(output_path / "model_similarity.png"))
    
    plot_variance_analysis(analyzer, top_n,
